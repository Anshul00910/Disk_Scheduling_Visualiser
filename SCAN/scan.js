document.addEventListener('DOMContentLoaded', function () {
    ...
    const directionSelect = document.getElementById('direction'); // Get direction element
    ...
    let chart = new CanvasJS.Chart("chart-container", {
      ...
      title: {
        text: "SCAN Disk Scheduling Visualization",
        fontColor: "#00bcd4",
        fontSize: 20
      },
      axisX: {
    ...
    visualizeBtn.addEventListener('click', function () {
      ...
      const results = calculateSCAN(requestSequence, initialHead, direction);
    ...
    function calculateSCAN(requests, head, direction) {
      const diskSize = 200;
      ...
      if (direction === "right") {
        ...
        fullSequence = right.concat([diskSize - 1], left);
      } else {
        left.sort((a, b) => b - a);   // descending
        right.sort((a, b) => a - b);  // ascending
        fullSequence = left.concat([0], right);
      }
