document.addEventListener('DOMContentLoaded', function () {
    ...
    const directionSelect = document.getElementById('direction');  // Get the direction selector
    ...
    let chart = new CanvasJS.Chart("chart-container", {
      ...
      title: {
        text: "C-SCAN Disk Scheduling Visualization",
        fontColor: "#00bcd4",
        fontSize: 20
      },
      axisX: {
    ...
    visualizeBtn.addEventListener('click', function () {
      ...
      const direction = directionSelect.value;  // Get selected direction
      ...
      const results = calculateCSCAN(requestSequence, initialHead, direction);
    ...
    function calculateCSCAN(requests, head, direction) {
        const diskSize = 200;
        let totalSeekTime = 0;
        ...
        if (direction === "right") {
          ...
          fullSequence = right.concat([diskSize - 1, 0], left);
        } else { // left direction
          left.sort((a, b) => b - a);    // descending
          right.sort((a, b) => b - a);   // descending
          fullSequence = left.concat([0, diskSize - 1], right);
        }
