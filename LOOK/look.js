document.addEventListener('DOMContentLoaded', function () {
    ...
    let chart = new CanvasJS.Chart("chart-container", {
      ...
      title: {
        text: "LOOK Disk Scheduling Visualization",
        fontColor: "#00bcd4",
        fontSize: 20
      },
      axisX: {
    ...
    visualizeBtn.addEventListener('click', function () {
      ...
      const results = calculateLOOK(requestSequence, initialHead, direction);
    ...
    function calculateLOOK(requests, head, direction) {
      let totalSeekTime = 0;
      ...
      if (direction === "right") {
        fullSequence = right.concat(left.reverse()); // Move right first, then left
      } else {
        fullSequence = left.reverse().concat(right); // Move left first, then right
      }
